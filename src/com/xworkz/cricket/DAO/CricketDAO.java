...
public abstract class CricketDAO {
	private CricketDTO[] dto = new CricketDTO[5]; // initialize array
	...
	public boolean save(CricketDTO dtos) {
		...
		// if duplicate found then cannot be saved
	...
	// finding team by country's name method
	...
	// finding team using country name and captain method
	public boolean findTeamByCountryNameAndCaptainName(String country, String captain) {
		...
		for (int z = 0; z < this.dto.length; z++) {
			if (this.dto[z] != null && this.dto[z].getCountry().equals(country)
					&& this.dto[z].getCaptain().equals(captain)) {
	...
	// finding team using country, catain name and sponsor method
	public boolean findTeamByCountryCaptainAndSponsor(String country, String captain, String sponsor) {
		...
		for (int z = 0; z < this.dto.length; z++) {
			if (this.dto[z].getCountry().equals(country) && this.dto[z].getCaptain().equals(captain)
					&& this.dto[z].getSponsor().equals(sponsor)) {
				System.out.println("team found at index" + z);
				return true;
			}
		}
	...
	// finding number of teams method
